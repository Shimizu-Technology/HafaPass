          ...
          {codes.map(pc => (
            <div key={pc.id} className="card p-5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => toggleActive(pc)} className="p-2 rounded-lg hover:bg-neutral-100 transition-colors" title={pc.active ? 'Deactivate' : 'Activate'}>
                  {pc.active ? <ToggleRight className="w-5 h-5 text-emerald-500" /> : <ToggleLeft className="w-5 h-5 text-neutral-400" />}
                </button>
                <button onClick={() => deleteCode(pc)} className="p-2 rounded-lg hover:bg-red-50 text-neutral-400 hover:text-red-500 transition-colors">

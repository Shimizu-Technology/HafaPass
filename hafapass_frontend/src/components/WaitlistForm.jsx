  ...
  if (result) {
    return (
      <div className="card p-6 text-center">
        ...
        <h3 className="text-lg font-semibold text-neutral-900 mb-1">You're on the Waitlist!</h3>
        <p className="text-neutral-600">You're <span className="font-bold text-brand-600">#{result.position}</span> in line</p>
        <p className="text-sm text-neutral-500 mt-2">We'll email you at <span className="font-medium">{result.email}</span> when tickets become available.</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="card p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-neutral-900">Join the Waitlist</h3>
        </div>
        <p className="text-sm text-neutral-500 mb-4">Get notified when tickets become available.</p>
      ...
      {/* Check Status */}
      <div className="card p-5">
        <h4 className="text-sm font-semibold text-neutral-900 mb-2">Already on the waitlist?</h4>
                ...
                {existingEntries.map(entry => (
                  <div key={entry.id} className="flex items-center justify-between text-sm bg-neutral-50 rounded-lg p-3">
                    <span className="text-neutral-700">
                      Position <span className="font-bold text-brand-600">#{entry.position}</span>
                      {entry.status !== 'waiting' && (
                        <span className="ml-2 text-xs font-medium text-neutral-500 uppercase">{entry.status}</span>
                      )}
                    </span>
                    {(entry.status === 'waiting' || entry.status === 'notified') && (
                      <button onClick={handleLeave} className="text-xs text-red-500 hover:text-red-700 font-medium">Leave</button>
                    )}
                  </div>
                ))}
              </div>

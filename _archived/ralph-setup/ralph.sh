...
for ((i=1; i<=MAX_ITERATIONS; i++)); do
  ...
  # Pipe the prompt to avoid argument parsing issues with @ references
  # --dangerously-skip-permissions allows full sandbox access (npm, agent-browser, etc.)
  result=$(cat PROMPT.md | claude -p --dangerously-skip-permissions 2>&1) || true
